...
module.exports = (req, res) => {
  ...
  const { name, author, isFavorite, publishYear, publishHouse, pagesNumber, genres, originalLanguage, img } = req.body;
  const permisbleKeys = "name, author, isFavorite, publishYear, publishHouse, pagesNumber, genres, img, originalLanguage".split(", ");
  ...
  if (typeof img !== "string") return utils.error(res, 400, "author attribute should be type `string`");
  ...
  db.get("books_expanded")
    .push({ ...newBook, ...expanded })
    .write();
